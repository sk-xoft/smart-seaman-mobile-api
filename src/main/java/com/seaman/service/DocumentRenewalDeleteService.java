...
public class DocumentRenewalDeleteService {
    private final DocumentRenewalFoundationRepository repository;
    private final HttpServletRequest httpServletRequest;

    @Transactional
    public DocumentRenewalDeleteResponse delete(String requestNoInput) {
        ...
        repository.softDeleteRequest(request.getId());
        repository.appendTransaction(request.getId(), DocumentRenewalAction.CANCEL,
                DocumentRenewalStatus.PAYMENT_PENDING,
                DocumentRenewalStatus.CANCELLED,
                "Renewal request deleted by applicant", user.getMobileUuid());

        return new DocumentRenewalDeleteResponse(requestNo,
                DocumentRenewalStatus.PAYMENT_PENDING.name(),
                DocumentRenewalStatus.CANCELLED.name(),
                DocumentRenewalAction.CANCEL.name());
    }
